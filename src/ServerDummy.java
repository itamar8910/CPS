    ...
    if(DO_CONJOBS) { 
	    new Thread(()-> {
	    	while(true) {
		    	...
		    	List<User> allUsers = ItamarDB.getInstance().getAllUsers();
		    	...
		    	for(User user : allUsers) {
		    		if(user.getSubscriptionParams().getParam("type").equals("orderedOneTimeParking")) {
		    			...
		    			if(System.currentTimeMillis() > orderTimeUnix && System.currentTimeMillis() < orderTimeUnix + 60*1000) {
		    				//update stats
		    				addToStatistics(ItamarDB.getInstance().getVehicleParkingLot(user.getVehicleID()), 1, 0, 0, 0);
		    				//notify via email
	    	...
	    	while(true) {
		    	...
		    	for(User user : allUsers) {
		    		...
		    		if(subscriptionParams.getParam("type").equals("routineSubscription") || subscriptionParams.getParam("type").equals("fullSubscription")) {
		    			...
		    			if(numDaysAgo > 28) {
		    				ItamarDB.getInstance().removeUser(user.getUserID());
		    				continue;
		    			}else if(numDaysAgo >= 21) {
		    				sendEmailToCostumerForBeginLate(user.getEmail(), "Please notice: your subscription is about to end in a week");
		    			}
		    			
		    			handleCallParkingAlgoOrderForSubscription(user.getUserID(), user.getVehicleID(), ItamarDB.getInstance().getVehicleParkingLot(user.getVehicleID()), subscriptionParams);
		    		}	
		    	}
...
//  }
	...
	final boolean CALL_ALGO = false;
	  ...
	  private void callParkingAlgoEnter(String parkingLot, String vehicleID, long leaveTime) {
		  ...
		  final int width = ItamarDB.getInstance().getParkingLotWidth(parkingLot);
		  JSONObject data = ItamarDB.getInstance().getParkingLotJsonData(parkingLot);
		  JSONArray start;
		try {
	 ...
	 private void initParkingLotData(String parkingLotName) {
		  final int width = ItamarDB.getInstance().getParkingLotWidth(parkingLotName);
		...
		try {
			...
			result.put("parkingData", new JSONArray(alg.generateDBString()));
			result.put("statusData", new JSONArray(alg.generateStatusString()));
			ItamarDB.getInstance().updateParkingLotData(parkingLotName, result.toString());
			//ItamarDB.getInstance().updateParkingLotData(parkingLotName, result.toString());
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	 ...
	 private void addToStatistics(String parkingLotName, int lateDelta, int cancelDelta, int arrivedDelta, int numDisabledDelta) {
		 int parkingLotID = ItamarDB.getInstance().getParkingLotIDByName(parkingLotName);
		 ...
		 ItamarDB.getInstance().initStatsIfDoesntExists(parkingLotID, todayUnixTime);
		 ItamarDB.getInstance().addToDailyStats(parkingLotID, todayUnixTime, lateDelta, cancelDelta, arrivedDelta, numDisabledDelta);
		 
	 }
	 
	  private Params handleClientPhysicalOrder(Params params) {
		  boolean isInTable = ItamarDB.getInstance().isInTable("Users", "userID", params.getParam("ID"));
		  if(isInTable){
		  ...
		  ItamarDB.getInstance().addUser(params.getParam("ID"), params.getParam("vehicleID"), params.getParam("email"), type);

		  handleAddVehicleToDB(params.getParam("ID"), params.getParam("parkingLot"), params.getParam("vehicleID"));//ItamarDB.getInstance().addVehicle(params.getParam("ID"), params.getParam("vehicleID"), params.getParam("parkingLot"), startTime, leaveTime);
	  ...
	  private double calcPriceUpfrontForOneTimeOrder(String parkingLot, Params subscriptionParams) {
			...
			List<Integer> prices = ItamarDB.getInstance().getPrices(parkingLot);
...
//		  final long leaveTime =Utils.dateAndTimeToMillis(params.getParam("leaveDate"), params.getParam("leaveTime"));
	  ...
	  private Params handleRoutineSubscription(Params params) {
		  ...
		  int subscriptionID = ItamarDB.getInstance().getIndexIDOfUser(params.getParam("ID"));
		  typeParams.addParam("subscriptionID", String.valueOf(subscriptionID));
		  ItamarDB.getInstance().updateUserType(params.getParam("ID"), typeParams.toString());
		  Params resp = Params.getEmptyInstance();
		  resp.addParam("status", "OK");
	...
	private void handleCallParkingAlgoOrderForSubscription(String userID, String vehicleID, String parkingLot, Params subscriptionParams) {
		...
		if(System.currentTimeMillis() > subscriptionStartTimeMS && Utils.isInLastMonth(subscriptionStartTimeMS)) {
			...
			if(parkingLot.equals("**ANY**")) { // if is full subscription, need to order in all parking lots
				List<String> allParkingLots = ItamarDB.getInstance().getAllParkingLotNames();
...
//			return (int)(numHours * prices.get(2) * prices.get(1));
			...
			prices = ItamarDB.getInstance().getPrices("Default");
	...
	private void handleAddVehicleToDB(String userID, String parkingLot, String vehicleID) {
		...
		//TODO: support multiple vehicles
		//String vehicleID = ItamarDB.getInstance().getUserVehicleID(userID);
		ItamarDB.getInstance().addVehicle(userID, vehicleID, parkingLot, 0, 0, false);
	...
	private void addToUserMoney(String userID, double amount) {
		ItamarDB.getInstance().addToUserMoney(userID, (amount));
	}

	private List<Integer> getPrices(String parkingLot){
		return ItamarDB.getInstance().getPrices(parkingLot);
	}

	private String updateVehicleEntered(String userID, String vehicleID, String parkingLot, Params subscriptionTypeParams) {
		boolean isInTable = ItamarDB.getInstance().isInTable("Vehicles", "vehicleID", vehicleID);
		if(!isInTable) {
		...
		ItamarDB.getInstance().updateVehicleStartLeaveTimes(vehicleID, currentUnixTime, expectedLeaveTimeUnix);
		ItamarDB.getInstance().updateVehicleIsParking(vehicleID, "true");
...
//		//TODO: impl. Adds all vehicles to Vehicles Table based on users (handles orders, routine subscriptions, full subscription, etc.)
	...
	private Params handleClientEnter(Params params) {
		...
		String userID = ItamarDB.getInstance().getUserIdFromVehicleID(vehicleID);
		String subscriptionParamsStr = ItamarDB.getInstance().getUserSubscriptionTypeStr(userID);
...
//			}
	...
	private Params handleClientLeave(Params params) {
		...
		boolean isVehicleInParking = ItamarDB.getInstance().getIsVehicleInParking(vehicleID);
		if(!isVehicleInParking) {
			return Params.getEmptyInstance().addParam("status", "BAD").addParam("message", "Vehicle is not currently parked");
		}
		ItamarDB.getInstance().updateVehicleIsParking(vehicleID, "false");
		String userID = ItamarDB.getInstance().getUserIdFromVehicleID(vehicleID);
		String subscriptionParamsStr = ItamarDB.getInstance().getUserSubscriptionTypeStr(userID);
		Params subscriptionParams = new Params(subscriptionParamsStr);
		...
		if(subscriptionParams.getParam("type").equals("physicalOrder") || subscriptionParams.getParam("type").equals("orderedOneTimeParking")){
			ItamarDB.getInstance().removeUser(userID);		
		}
		//from vehicle from vehicles table
		ItamarDB.getInstance().removeVehicle(vehicleID);
	...
	private double calcPriceToPay(String parkingLot, String vehicleID, Params subscriptionParams) {
		...
		if(subscriptionParams.getParam("type").equals("physicalOrder")) {
			...
			long startParkTimeUnix = ItamarDB.getInstance().getVehicleStartParkTime(vehicleID);
	...
	private Params handleClientCancelOrder(Params params) {
		String vehicleID = params.getParam("vehicleID");
		String userID = ItamarDB.getInstance().getUserIDByVehicleID(vehicleID);
		String parkingLot = ItamarDB.getInstance().getVehicleParkingLot(vehicleID);
		Params subscriptionParams = new Params(ItamarDB.getInstance().getUserSubscriptionTypeStr(userID));
		if (!subscriptionParams.getParam("type").equals("orderedOneTimeParking")) {
...
//		return resp;
	...
	private Params handleGNumOfSubscribers(Params params) {
		...
		String parkingLotName = ItamarDB.getInstance().getParkingLotNameByID(parkingLotID);
		List<User> users = ItamarDB.getInstance().getAllUsers();
		int count = 0;
		for(User user : users) {
			if(!ItamarDB.getInstance().getVehicleParkingLot(user.getVehicleID()).equals(parkingLotName)) {
				continue;
			}
	...
	private Params toggleDisableSpot(Params params) {
		...
		try {
			...
			int numCols = ItamarDB.getInstance().getParkingLotWidth(parkingLotName);
			JSONObject parkingData = ItamarDB.getInstance().getParkingLotJsonData(parkingLotName);
			...
			for(int index = 0; index < statusData.length(); index++) {
				...
				if(spotStatus.getInt("i") == i && spotStatus.getInt("j") == j && spotStatus.getInt("k") == k) {
					spotStatus.put("status", String.valueOf((value).equals("true") ? 'i' : spotStatus.get("status")));
				}
				newStatusData.put(spotStatus);
			}
			parkingData.put("statusData", newStatusData);
			ItamarDB.getInstance().updateParkingLotData(parkingLotName, parkingData.toString());
	...
	private Params reserveSpot(Params params) {
		...
		try {
			...
			for(int index = 0; index < statusData.length(); index++) {
				JSONObject spotStatus = statusData.getJSONObject(index);
				if(spotStatus.getInt("i") == i && spotStatus.getInt("j") == j && spotStatus.getInt("k") == k) {
					spotStatus.put("status", String.valueOf('s'));
				}
				newStatusData.put(spotStatus);
			}
...
private Params isParkingLotFull(Params params) {
	String parkingName = params.getParam("name");
	String fullRespStr = ItamarDB.getInstance().canBeInParking(Params.getEmptyInstance().addParam("facName", parkingName));
...
//	    		client.sendToClient(resp.toString());
	    		...
	    		client.sendToClient(resp.toString());
	    	}else if(params.getParam("action").equals("getParkingLotWidth")) {
	    		System.out.println("getParkingLotWidth");
	    		int parkingLotWidth = ItamarDB.getInstance().getParkingLotWidth(params.getParam("name"));
	    		Params resp = Params.getEmptyInstance().addParam("status", "OK").addParam("width", String.valueOf(parkingLotWidth));
	    		client.sendToClient(resp.toString());
